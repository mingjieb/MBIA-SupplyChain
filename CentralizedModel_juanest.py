...
def setup_model(params,time_neutral,ratios,unmet_penalty,relax):
    ...
    #penalty
    m._rho_I = encode_params(param=params['rho_I'],override=unmet_penalty) #penalty cost for unmet demand
    ...
    for idx,val in m._r.items():
        if val == 1:
            if idx[0] not in all_prods:
                all_prods.append(idx[0])
                m._prod_stucture[idx[0]] = {}
            m._prod_stucture[idx[0]][idx[1]] = val
    ...
    m._x = m.addVars(m._valid_i_k, name='demand_satisfied',lb=-GRB.INFINITY,ub=GRB.INFINITY) #unmet demand
    ...
    if time_neutral == False:
        '''
        m.setObjective(gp.quicksum(m._c[tuple([i,j,k])] * m._y[i,j,k] for i, j in m._E for k in m._K) +
                            gp.quicksum(m._h[tuple([i,k])] * m._I[i,k] for i in m._V for k in m._K) +
                            gp.quicksum(m._e[tuple([i,k])] * m._p[i,k] for i in m._V for k in m._K) +
                            gp.quicksum(m._f[tuple([i,j,k])] * m._beta[i,j,k] for i,j in m._E for k in m._K) +
                            gp.quicksum(m._phi[i] * m._zeta[i] for i in m._V) + 
                            gp.quicksum( m._rho_I[i, k] * m._delta_I[i, k] for i in m._V for k in m._K) + 
                            gp.quicksum(m._rho_d[i, k] * m._delta_d[i, k] for i in m._V for k in m._K ) +
                            gp.quicksum(m._w[i,j,k] for i, j in m._E for k in m._K) 
                            ,GRB.MINIMIZE)
        '''
    ...
    m.addConstrs(((gp.quicksum(m._y[i,j,k] for _,j,kk in m._valid_arcs if _ == i if kk == k) - gp.quicksum(m._y[j,i,k] for j,_,kk in m._valid_arcs if _ == i if kk == k) 
                    + gp.quicksum( m._r[k1,k]*m._p_cap[i,k1]*m._p[i,k1] for _,k1 in m._valid_i_k if _ == i) - m._p_cap[i,k]*m._p[i,k] 
                    == 
    ...
    #m.addConstrs((m._x[i,k] >= m._d[i,k] for i in m._V for k in m._K),name='Demand constraints')
...
class CentralizedSinglePeriod:
    def __init__(self, params,time_neutral=False,ratios=[],unmet_penalty=[],relax=False):
        self.time_neutral = time_neutral
        self.results = None
        self.solved = False
        self.m = setup_model(params,time_neutral,ratios,unmet_penalty,relax)
    ...
    def induce_disruption(self,network,disruptions=None,response_type='s'):
        #Modifying model to incorporate the cost of changes in network structure to control induced disruptions
        #Adding current networks structure
        self.m._exist_beta = network['exist_beta']
        self.m._exist_zeta = network['exist_zeta']

        #Adding network structure change penalties
        self.m._Rho_beta = network['penalty_beta']
        self.m._Rho_zeta = network['penalty_zeta']

        #Adding variables of network change
        self.m._delta_V = self.m.addVars(self.m._V,vtype=GRB.BINARY,name='Vertex change')
        self.m._delta_E = self.m.addVars(self.m._E,vtype=GRB.BINARY,name='Arcs change')

        #Inducing network disruptions as constraints on the network flow feasible set 


        #Introducing network structure change penalty computation constraints
        self.m.addConstrs((self.m._delta_V[i] >= self.m._zeta[i] - self.m._exist_zeta[i] for i in self.m._V),name='Vertex addition penalty')
        self.m.addConstrs((self.m._delta_V[i] >= self.m._exist_zeta[i] - self.m._zeta[i] for i in self.m._V),name='Vertex removal penalty')
        self.m.addConstrs((self.m._delta_E[i,j] >= self.m._beta[i,j,k] - self.m._exist_beta[i,j,k] for i,j in self.m._E for k in self.m._K),name='Arc addition penalty')
        self.m.addConstrs((self.m._delta_E[i,j] >= self.m._exist_beta[i,j,k] - self.m._beta[i,j,k] for i,j in self.m._E for k in self.m._K),name='Arc removal penalty')

        #Modifying objective function to consider the change in network structure
        self.m.setObjective(gp.quicksum(self.m._c[tuple([i,j,k])] * self.m._y[i,j,k] for i, j in self.m._E for k in self.m._K) +
                        gp.quicksum(self.m._h[tuple([i,k])] * self.m._I[i,k] for i in self.m._V for k in self.m._K) +
                        gp.quicksum(self.m._e[tuple([i,k])] * self.m._p[i,k] for i in self.m._V for k in self.m._K) +
                        gp.quicksum(self.m._f[tuple([i,j,k])] * self.m._beta[i,j,k] for i,j in self.m._E for k in self.m._K) +
                        gp.quicksum(self.m._phi[i] * self.m._zeta[i] for i in self.m._V) + 
                        gp.quicksum( self.m._rho_I[i, k] * self.m._delta_I[i, k] for i in self.m._V for k in self.m._K) + 
                        gp.quicksum(self.m._rho_d[i, k] * self.m._delta_d[i, k] for i in self.m._V for k in self.m._K) +
                        gp.quicksum(self.m._Rho_beta[i,j]*self.m._delta_E[i,j] for i,j in self.m._E)+
                        gp.quicksum(self.m._Rho_zeta[i]*self.m._delta_V[i] for i in self.m._V)
                        ,GRB.MINIMIZE)
    ...
    def get_network_structure(self,penalties={'v':1,'e':1}):
        network = {}
        network['exist_beta'] = self.beta_sol
        network['exist_zeta'] = self.zeta_sol
        network['penalty_beta'] = {}
        for idx in self.beta_sol.keys():
            network['penalty_beta'][idx] = penalties['e']
        network['penalty_zeta'] = {}
        for idx in self.zeta_sol.keys():
            network['penalty_zeta'][idx] = penalties['v']
        return network
    ...
    def solve(self,disruption_response=False,silent=False):
        ...
        if d[stat] == 'OPTIMAL': 
            ...
            if self.time_neutral == False:
                ...
                for key,val in self.y_sol.items():
                    if val > 0:
                        ...
                        active_flows[key]['lead_time'] = self.m._l[key]
                ...
                for part in tier_members['Part']:
                        ...
                        for k in ks:
                            o[(part,k)] = 0 
                sequence = ['Manuf','Dist','Retail'] #We just need to consider those that take flow
                transformers = ['Manuf'] #Considering those vertices that transform 
            ...
            penalty_demand = 0 
            for i,k in self.m._valid_i_k:
                penalty_demand += self.m._rho_d[i,k]*self.delta_d_sol[i,k]
            reactCost = 0

            if silent == False:
                print("Objective function is: " + str(self.m.ObjVal))
                print("The production cost is: " + str(prodCost))
                print("The transportation cost is: " + str(transCost))
                self.results['Production cost'] = float(prodCost)
                self.results['Flow cost'] = float(transCost)
                ...
                if abs(penalty_demand) < 1e-5:
                    ...
                    for k in self.m._K:
                        for i in self.m._V:
                            if self.delta_d_sol[i, k] > 1e-4:
                                print(str((i, k)) + ": " + str(self.delta_d_sol[i, k]))

            if disruption_response == True:
                #Extracting response variable values in solution
                self.delta_E_sol = self.m.getAttr('x',self.m._delta_E)
                self.delta_V_sol = self.m.getAttr('x',self.m._delta_V)
                for i in self.m._V:
                    reactCost += self.m._Rho_zeta[i]*self.delta_V_sol[i]
                for i,j in self.m._E:
                    reactCost += self.m._Rho_beta[i,j]*self.delta_E_sol[i,j]
                print("The reaction cost is: " + str(reactCost))
                print("Total changes of vertices: " + str(sum(self.delta_V_sol[i] for i in self.m._V)))
                if sum(self.delta_V_sol[i] for i in self.m._V) > 1e-2:
                    for i in self.m._V:
                        if (self.delta_V_sol[i]) > 1e-4:
                            if (self.zeta_sol[i]) > 1e-4:
                                print(str(i) + ": Added")
                            else:
                                print(str(i) + ": Removed")
                print("Total changes of edges: " + str(
                    sum((self.delta_E_sol[i, j]) for i, j in self.m._E)))
                if sum((self.delta_E_sol[i, j]) for i, j in self.m._E) > 1e-2:
                    for i, j in self.m._E:
                        if (self.delta_E_sol[i, j]) > 1e-4:
                            if (self.beta_sol[i, j]) > 1e-4:
                                print(str((i, j)) + ": Added")
                            else:
            ...
            if silent == False:
                ...
                production = []
                for i,k in self.m._valid_i_k:
                        if abs(self.p_sol[i, k]) > 1e-8:
                            print(str((i, k)) + ": " + str(self.p_sol[i, k]))
                            prod = {'Agent': i, "Product": k, "Value": float(self.p_sol[i, k])}
                            production.append(prod)
                self.results['Productions'] = production

                print("Flows:")
                flows = []
                for i,j,k in self.m._valid_arcs:
                    if abs(self.y_sol[i,j, k]) > 1e-8:
                        print(str((i,j,k)) + ": " + str(self.y_sol[i,j,k]))
                        fl = {'Source': i, "Dest": j, "Product": k, "Value": float(self.y_sol[i,j,k])}
                        flows.append(fl)
                self.results['Flows'] = flows
    ...
    def solution_visualization(self):
        ...
        for k,i in self.y_sol.items():
            if i > 0:
                if (k[0],k[1]) in used_arcs:
                    ...
                    flow_arcs_number_1.append(i/300)
        ...
        nx.draw(self.G, pos,node_size=4000,width=2,with_labels=True, vmin=0, vmax=max(values),
                                node_color=values,
                                font_size=29,
                                font_color='k',
                                edge_color='white',
    ...
    def condensed_statistics_single(self,stats):
        #plotting histogram of lateness of products
        stats = pd.concat(stats, ignore_index=True)
        stats['Type'] = stats['j'].map(self.m._V_type)
        print(stats)
        hist_data = stats[stats['Type']=='Retail'][['lateness','flow','k']].groupby(['lateness','k']).sum().unstack().fillna(0)
        for c in hist_data.columns:
            col = hist_data[c]
            sum = col.sum()
            for r in col.index:
                hist_data.loc[r,c] = int(hist_data.loc[r,c]/sum*100)

        legends = []
        colors = []
        for l in hist_data.columns:
            prod = l[1]
            colors.append(self.color_map[prod])
            if prod[-1].isnumeric():
                prod = f'{prod[:-1]}$_{prod[-1]}$'
            legends.append(prod)
            
        ax = plt.figure(figsize=(10, 6)).add_subplot(111)

        hist_data.plot.bar(ax=ax,stacked=False,color=colors)
        hatches = ['/','-','x','O','-','.','+','*','o']
        count = 0
        bars = ax.patches
        for bar in bars:
            if count <= int(len(bars)/2)-1:
                h = 1
            else:
                h = 0
            bar.set_hatch(hatches[h])
            
            count += 1
        plt.xlabel('Delivery lateness to customers')
        plt.ylabel(r'$\%$ of total flow')
        plt.legend(legends)
        plt.xticks(rotation=0)
        plt.savefig('baseline_solution_simulation.pdf',bbox_inches='tight')
        plt.show()

        hist_data = stats[['lateness','flow','k']].groupby(['lateness','k']).sum().unstack().fillna(0)
        for c in hist_data.columns:
            col = hist_data[c]
            sum = col.sum()
            for r in col.index:
                hist_data.loc[r,c] = hist_data.loc[r,c]/sum
        ...
        ax = plt.figure(figsize=(10, 6)).add_subplot(111)
        hist_data.plot.bar(ax=ax,stacked=False,color=colors)
        plt.xlabel('Lateness of arrival to downstream vertex')
        plt.ylabel(r'$\%$ of units')
        plt.legend(legends)
        plt.xticks(rotation=0)
        plt.show()
    ...
    def condensed_statistics_compare(self,simulations,colors,filename):
        '''
        def func(x, a, b, c):
