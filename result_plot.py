...
with open('CentralizedResults/Case4-Penalty/Centralized_results.json') as f:
    centralized_results = json.load(f)
with open('CentralizedResults/Case4-Penalty/runtime.json') as f:
    centralized_runtime = json.load(f)
with open('CentralizedResults/Case4-Penalty/communication.json') as f:
...
def reformulate_results(distributed_data, centralized_results):
    agent_attributes = {"g_in": {}, "g_out": {}, "g_com": {}, "P": {}, "Q": {}, "F": {}, "l": {}, "n": {}, "depth":{}}
...
def plot_attributes(mfg_list, agent_attributes):
    ...
    # Plot in-flow, out-flow, and communication degree
    g_in = [agent_attributes["g_in"][mfg_list[i]]+agent_attributes["g_out"][mfg_list[i]] for i in range(len(mfg_list))]
    ax1.scatter(x, g_in, marker='+', color='#6699CC', label="Connectivity")
    ax1.set_ylabel("Connectivity", fontsize=12)
    ax1.set_title("Sum of In-degree and out-degree", fontsize=12)

    # Plot capability redundancy and capacity sufficiency
    p = [agent_attributes["P"][mfg_list[i]] for i in range(len(mfg_list))]
    ax2.scatter(x, p, marker='+', color='#6699CC', label=r'$P_i(m)$')
    ax2.set_ylabel(r'$P_i(m)$', fontsize=12)
    ax2.set_title("Capability redundancy", fontsize=12)

    q = [agent_attributes["Q"][mfg_list[i]] for i in range(len(mfg_list))]
    ax3.scatter(x, q, marker='x', color='#EEB479', label=r'$Q_i(m)$')
    ax3.set_ylabel( r'$Q_i(m)$', fontsize=12)
    ax3.set_title("Capacity sufficiency", fontsize=12)

    # Plot production need and contributions to final products
    n = [agent_attributes["n"][mfg_list[i]] for i in range(len(mfg_list))]
    ax4.scatter(x, n, marker='x', color='#EEB479', label=r'$n_i(k)$')
    ax4.set_xlabel("Disruption scenarios", fontsize=12)
    ax4.set_ylabel(r'$n_i(k)$', fontsize=12)
    ax4.set_title("Product impact on final products", fontsize=12)
...
def demand_satisfaction_plots(distributed_data, centralized_results, mfg_list, n_dis_unmet, n_cen_unmet):
    ...
    cen_delta_d = [centralized_results[mfg_list[i]]["results"]["delta_d"] for i in n_dis_unmet]
...
def network_change_plots(distributed_data, centralized_results, mfg_list, n_dis_met, n_cen_met):
    ...
    # Scatter plot of over capacity cost
    ax1 = plt.subplot(311)
    ax2 = plt.subplot(312)
    ax3 = plt.subplot(313)
    x = [i + 1 for i in n_dis_met]
    ...
    ax1.scatter(x, y_fd, marker='+', color='#496C88', label=r'$O_f$' ', distributed')
    ax1.scatter(x, y_fc, marker='x', color='#E88482', label=r'$O_f$' ', centralized')
    ax1.axhline(y=0.0, color='grey', linestyle='--')
    ...
    ax2.scatter(x, y_pd, marker='+', color='#496C88', label=r'$O_p$' ', distributed')
    ax2.scatter(x, y_pc, marker='x', color='#E88482', label=r'$O_p$' ', centralized')
    ax2.axhline(y=0.0, color='grey', linestyle='--')
    ax2.set_ylabel(r'$O_p$', fontsize=12)
    ax2.set_title("Cost of over-capacity productions", fontsize=12)

    # Total over-capacity cost
    y_totald = [distributed_data[mfg_list[i]]["results"]["O_c_flow"] + distributed_data[mfg_list[i]]["results"][
        "O_c_production"]
                for i in n_dis_met]
    y_totalc = [centralized_results[mfg_list[i]]["results"]["O_c_flow"] + centralized_results[mfg_list[i]]["results"][
        "O_c_production"]
                for i in n_dis_met]
    # ax3.plot(x, y_totald, marker='.', color='#496C88', label=r'$O_{total}$' ', distributed', linewidth=2)
    # ax3.plot(x, y_totalc, marker='.', color='#E88482', label=r'$O_{total}$' ', centralized', linewidth=2)
    ax3.scatter(x, y_totald, marker='+', color='#496C88', label='Distributed')
    ax3.scatter(x, y_totalc, marker='x', color='#E88482', label='Centralized')
    ax3.axhline(y=0.0, color='grey', linestyle='--')
    ax3.set_xlabel("Disruption scenarios " r'($\Delta^d=0$)', fontsize=12)
    ax3.set_ylabel(r'$O_{total}$', fontsize=12)
    ax3.set_title("Total cost of over-capacity flows and productions", fontsize=12)
...
# for (cat_name, cat), axes_col in zip(res.groupby('sex'), axes_cols):
...
#
# labels = metrics
# bp1 = ax1.boxplot([dis_ll[metric] for metric in metrics], patch_artist=True, labels=labels)
# ax1.set_title('low low', fontsize=12)
# ax1.set_xticklabels(labels=labels, Fontsize=12)
#
#
# bp2 = ax2.boxplot([dis_lm[metric] for metric in metrics], patch_artist=True, labels=labels)
# ax2.set_title('low mid', fontsize=12)
# ax2.set_xticklabels(labels=labels, Fontsize=12)
#
#
# bp3 = ax3.boxplot([dis_lh[metric] for metric in metrics], patch_artist=True, labels=labels)
# ax3.set_title('low high', fontsize=12)
# ax3.set_xticklabels(labels=labels, Fontsize=12)
#
#
# bp4 = ax4.boxplot([dis_hl[metric] for metric in metrics], patch_artist=True, labels=labels)
# ax4.set_title('high low', fontsize=12)
# ax4.set_xticklabels(labels=labels, Fontsize=12)
#
# bp5 = ax5.boxplot([dis_hm[metric] for metric in metrics], patch_artist=True, labels=labels)
# ax5.set_title('high mid', fontsize=12)
# ax5.set_xticklabels(labels=labels, Fontsize=12)
#
#
# bp6 = ax6.boxplot([dis_hh[metric] for metric in metrics], patch_artist=True, labels=labels)
# ax6.set_title('high high', fontsize=12)
# ax6.set_xticklabels(labels=labels, Fontsize=12)
#
# # colors = ['pink', 'lightblue']
# # for bplot in (bp1, bp2, bp3, bp4):
# #     for patch, color in zip(bplot['boxes'], colors):
